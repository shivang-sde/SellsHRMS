...
public interface TicketRepository extends JpaRepository<Ticket, Long> {
       ...
       List<Ticket> findByProjectIdAndIsActiveTrue(Long projectId);
       ...
       @Query("""
                         ...
                         WHERE t.organisation.id = :organisationId
                           AND (a.id = :employeeId OR t.createdBy.id = :employeeId)
                     """)
       List<Ticket> findAllVisibleToEmployee(@Param("organisationId") Long organisationId,
                     @Param("employeeId") Long employeeId);

       // Subordinate independent tickets: no project, created by OR assigned to any
       // employee in the given set
       @Query("""
                     SELECT DISTINCT t FROM Ticket t
                     LEFT JOIN t.assignees a
