let ticketId =
  window.ticketId || document.getElementById("taskTicketId")?.value || null;
...
async function loadTicketDetails() {
  try {
    const res = await ticketAPI.getById(
      ticketId,
      window.APP.ORG_ID,
    ...
    $("#ticketAssignees").text(
      (ticketData.assigneeNames || []).join(", ") || "-",
    );
...
async function editTask(id) {
  try {
    ...
    Object.entries(t).forEach(([k, v]) => {
      ...
      if (el.length) {
        if (el.attr("type") === "datetime-local" && v)
          el.val(v.replace(" ", "T"));
        else el.val(v);
      }
    });
...
async function editTicketWorkUpdate(id) {
  try {
    ...
    const t = await taskAPI.getById(id);
...
function renderTasks(tasks) {
  ...
  if (!tasks.length) {
    tbody.html(
      '<tr><td colspan="6" class="text-center text-muted py-3">No work updates yet</td></tr>',
    );
    return;
  }
