$(document).ready(function () {
  ...
  window.deleteSubject = function (subjectId) {
    if (
      !confirm(
        "Are you sure you want to delete this subject? All topics will be deleted.",
      )
    ) {
      return;
    }
  ...
  function loadSubjectWithTopics(subjectId) {
    $.ajax({
      ...
      error: function (xhr) {
        $("#topicsList").html(
          '<div class="alert alert-danger">Error loading topics</div>',
        );
      },
    });
  }
  ...
  function handleCreateTopic(e) {
    ...
    const formData = {
      ...
      attachmentUrl:
        $('#createTopicForm input[name="attachmentUrl"]').val() || null,
    };
