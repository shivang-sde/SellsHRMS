...
function renderSlipTable(slips) {
  ...
  slips.forEach((slip) => {
    const payslipdate = new Date(slip.fromDate);
    ...
    const row = `
            <tr>
                <td>${payslipdate.toLocaleString("default", { month: "long" })}</td>
                <td>${payslipdate.getFullYear()}</td>
                <td>${slip.netPay || "₹0.00"}</td>
  ...
  $.ajax({
    ...
    success: function (response) {
      if (response && response.salarySlipHtml) {
        ...
        $("#slipPreviewContent").html(
          '<p class="text-center text-muted py-5">No preview available.</p>',
        );
      }
    },
    error: function () {
      $("#slipPreviewContent").html(
        '<p class="text-center text-danger py-5">Error loading slip preview.</p>',
      );
    },
  });
}
